    ...
    try {
        ...
        const orderProducts = await Promise.all(
            items.map(async (item, index) => {
                ...
                console.log("specId:", cartItem.specId);
                ...
                const spec = product.specification.find(spec => spec._id.toString() === cartItem.specId.toString());

                if (!spec) {
                    throw new Error(`Specification not found for specId ${cartItem.specId} in product ${item.productId}`);
                }
